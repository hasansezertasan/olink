...
- **Seeded at `0.1.0`**: `pyproject.toml`, `src/olink/__init__.py`, and `.release-please-manifest.json` all start at `0.1.0` (mirrors ocom). Consequence: the first automated Release PR proposes the *next* bump from `0.1.0` (e.g. `0.1.1` for a fix, `0.2.0` for a feat), so `0.1.0` itself is the committed baseline, not an auto-published release. To make `0.1.0` the first published release instead, seed these three at `0.0.0` and add `"bump-minor-pre-major": true`.
