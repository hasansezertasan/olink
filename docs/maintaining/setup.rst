...
`Contributing Guidelines <https://github.com/hasansezertasan/olink/blob/main/.github/CONTRIBUTING.md>`_.
...
.. code-block:: sh
   ...
   gh repo edit hasansezertasan/olink \
     --enable-squash-merge \
...
.. code-block:: sh
   ...
   gh api repos/hasansezertasan/olink \
...
.. code-block:: sh
   ...
   gh api -X PUT repos/hasansezertasan/olink/actions/permissions/workflow \
...
.. code-block:: sh
   ...
   gh api repos/hasansezertasan/olink/actions/permissions/workflow \
...
.. code-block:: sh
   ...
   gh api --silent \
     repos/hasansezertasan/olink/dependency-graph/compare/main...main
...
- **PyPI Project Name:** ``olink``
- **Owner:** ``hasansezertasan``
- **Repository name:** ``olink``
...
.. code-block:: sh
   ...
   gh api -X PUT repos/hasansezertasan/olink/environments/publish \
     -F 'deployment_branch_policy[protected_branches]=false' \
     -F 'deployment_branch_policy[custom_branch_policies]=true'
   ids="$(gh api --paginate repos/hasansezertasan/olink/environments/publish/deployment-branch-policies \
     --jq '.branch_policies[].id')"
   for id in $ids; do
     gh api -X DELETE "repos/hasansezertasan/olink/environments/publish/deployment-branch-policies/$id"
   done
   gh api -X POST repos/hasansezertasan/olink/environments/publish/deployment-branch-policies \
...
.. code-block:: sh
   ...
   gh variable set ENABLE_PRIVATE_ATTESTATIONS --body true --repo hasansezertasan/olink
...
.. code-block:: sh
   ...
   policies="$(gh api --paginate repos/hasansezertasan/olink/environments/publish/deployment-branch-policies \
...
.. code-block:: sh
   ...
   gh secret set CODECOV_TOKEN --repo hasansezertasan/olink

**[CHECK]** A public repo needs no token; a private repo needs the secret:

.. code-block:: sh

   priv="$(gh repo view hasansezertasan/olink --json isPrivate --jq '.isPrivate' 2>/dev/null)"
   if [ "$priv" = "false" ]; then true
   elif [ "$priv" = "true" ]; then
     gh secret list --repo hasansezertasan/olink --json name --jq '.[].name' | grep -qx CODECOV_TOKEN
   else false; fi
...
.. code-block:: sh
   ...
   gh secret set GITLEAKS_LICENSE --repo hasansezertasan/olink
...
.. code-block:: sh
   ...
   owner="$(gh api repos/hasansezertasan/olink --jq '.owner.type' 2>/dev/null)"
   if [ "$owner" = "User" ]; then true
   elif [ "$owner" = "Organization" ]; then
     gh secret list --repo hasansezertasan/olink --json name --jq '.[].name' | grep -qx GITLEAKS_LICENSE
   else false; fi
...
.. code-block:: sh
   ...
   gh api -X POST repos/hasansezertasan/olink/pages \
...
.. code-block:: sh
   ...
   gh api repos/hasansezertasan/olink/pages \
...
.. code-block:: sh
   ...
   [ "$(gh repo view hasansezertasan/olink --json hasDiscussionsEnabled --jq '.hasDiscussionsEnabled')" = "true" ]
...
.. code-block:: sh
   ...
   gh secret list --repo hasansezertasan/olink --json name \
...
``.all-contributorsrc`` and the README **Contributors** section follow the
`all-contributors <https://allcontributors.org/>`_ specification. The bundled
``all-contributors.yml`` workflow regenerates the README table from
``.all-contributorsrc`` (on demand via *Actions → All Contributors → Run
workflow*, and automatically when ``.all-contributorsrc`` changes on the default
branch) and opens a PR with the result — this reuses the same *Allow GitHub
Actions to create and approve pull requests* setting release-please needs (the
"Let Actions open the release PR" step above), so no extra setup.

To add people, either edit ``.all-contributorsrc`` (or run the
`CLI <https://allcontributors.org/docs/en/cli/usage>`_, which is pinned in
``mise.toml``: ``mise exec -- all-contributors add <user> <contributions>``) and
let the workflow regenerate, or install the
`bot <https://allcontributors.org/docs/en/bot/usage>`_ for
``@all-contributors please add @user for code`` comments.

**[CHECK]** No extra check — covered by "Let Actions open the release PR" above.
...
``https://repology.org/project/olink/versions`` and verify that
...
``olink``. If it does, use the name shown by Repology in both
URLs below.
...
.. code-block:: markdown
   ...
   [![Packaging status](https://repology.org/badge/vertical-allrepos/olink.svg)](https://repology.org/project/olink/versions)
