...
Follow [Conventional Commits v1.0.0](https://www.conventionalcommits.org/en/v1.0.0/). Commit messages are the source of truth for releases: [release-please](https://github.com/googleapis/release-please) parses them on every push to `main` and maintains a "Release PR" with the computed version bump and changelog. Merging that PR creates the `vX.Y.Z` git tag (via `force-tag-creation`) and a draft GitHub release, after which CI builds, publishes to PyPI via Trusted Publishing, and un-drafts the release. The version itself is no longer a hand-edited (or release-please-edited) literal: `pyproject.toml` declares `dynamic = ["version"]`, and [hatch-vcs](https://github.com/ofek/hatch-vcs) (`[tool.hatch.version] source = "vcs"`) derives it from that git tag at build time, writing it to a generated, git-ignored `src/olink/_version.py` that is re-exported as `olink.__version__` (what `olink --version` reads). release-please only manages `CHANGELOG.md` and the release PR/tag — it no longer touches `pyproject.toml` or `src/olink/__init__.py`. The publish workflow checks out with `fetch-depth: 0` so hatch-vcs can see the tag when it computes the version.
